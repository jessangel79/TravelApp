...
class CurrencyServiceTestCase: XCTestCase {
    ...
    func testGetCurrencyShouldPostSuccessCallbackIfNoErrorAndCorrectData() {
        ...
        currencyService.getCurrency { (success, currency) in
            ...
            let symbolsJSON = symbols.map({(symbol, _) in symbol})
            XCTAssertEqual(symbolsJSON.sorted(), currency?.sorted())
    ...
    func testGetRateShouldPostFailedCallBackIfIncorrectData() {
        // Given
        let currencyService = CurrencyService(
            session: URLSessionFake(data: FakeResponseData.ratesIncorrectData, response: FakeResponseData.responseOK, error: nil))
    ...
    func testGetRateShouldPostSuccessCallbackIfNoErrorAndCorrectData() {
        ...
        currencyService.getRate(symbol: symbolPicked) { (success, rate) in
            ...
            let rates = ["USD": 1.119501]
            let ratesJSON = rates[symbolPicked]
            XCTAssertEqual(ratesJSON, rate)
